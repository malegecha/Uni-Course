...
int main (int argc, char **argv) {
	...
	for (int k = 0; k <TIME; k++ ) {
		...
		if ((world_rank / Pj) % 2 == 0) {
			...
			//	MPI_Recv(&map[world_rank][I][1][0], 1, row, neighbours[DOWN], 9, MPI_COMM_WORLD, &status);
		...
		if ((world_rank / Pj) % 2 == 0) {
		  ...
		  //	MPI_Recv(&map[world_rank][I][1][0], 1, row, neighbours[DOWN], 10, MPI_COMM_WORLD, &status);
		}
		...
		if ((world_rank / Pj) % 2 == 0) {
			...
			//MPI_Recv(&map[world_rank][1][1][0], 1, row, neighbours[UP],11, MPI_COMM_WORLD, &status);
		...
		if ((world_rank / Pj) % 2 == 0) {
			...
			//MPI_Recv(&map[world_rank][1][1][0], 1, row, neighbours[UP], 12, MPI_COMM_WORLD, &status);
		}
...
//			MPI_Recv(&map[world_rank][1][J][0] , 1, col, neighbours[RIGHT], 13, MPI_COMM_WORLD, &status);
		...
		if (world_rank %2 == 0) {
//			MPI_Recv(&map[world_rank][1][J][0] , 1, col, neighbours[RIGHT], 14, MPI_COMM_WORLD, &status);
		...
		if (world_rank % 2 == 0) {
			...
			//MPI_Recv(&map[world_rank][1][1][0] , 1, col, neighbours[LEFT], 15, MPI_COMM_WORLD, &status);
		...
		if (world_rank %2 == 0) {
			//MPI_Recv(&map[world_rank][1][1][0] , 1, col, neighbours[LEFT], 16, MPI_COMM_WORLD, &status);
